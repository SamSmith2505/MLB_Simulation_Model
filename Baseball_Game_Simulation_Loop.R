...
for (game in unique(lineup_dt$game_id)) {
  ...
  batter_result_frequency_dt = subset(batter_result_frequency, dataset %in% c("MLB 2019 Regular Season"))
  pitcher_result_frequency_dt = subset(pitcher_result_frequency, dataset %in% c("MLB 2019 Regular Season"))
  
  current_game[!(player %in% batter_result_frequency_dt$batter) & position != 1, player := "REPLACEMENT BATTER"]
  current_game[!(player %in% pitcher_result_frequency_dt$pitcher) & position == 1, player := "REPLACEMENT PITCHER"]
  ...
  game_bullpen_frequency_dt = subset(bullpen_result_frequency, team %in% c(home_team, away_team))
  game_bullpen_frequency_dt[, `:=`(game_id = game,
                                   home_away = ifelse(team == home_team, 'H', "A"))]
  game_pitcher_result_frequency_dt = rbindlist(list(game_pitcher_result_frequency_dt,
                                           game_bullpen_frequency_dt)) %>% setDT()
  
  game_historical_pitch_count = subset(pitch_count_historical, pitcher %in% game_pitcher_dt$player)
  game_historical_pitch_count = merge(game_historical_pitch_count,
                                      game_pitcher_dt[, .(player, home_away)],
                                      by.x = "pitcher",
                                      by.y = "player") %>% data.table()
